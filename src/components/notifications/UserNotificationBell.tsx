...
import { Bell, Filter, Search, Check, X, Trash2, MoreVertical, MessageCircle, Package, ShoppingCart, Gift, Settings } from 'lucide-react';
...
export const UserNotificationBell: React.FC = () => {
  ...
  const handleCreateTestNotifications = async () => {
    try {
      await userNotificationService.createTestNotifications('all', 3);
      await loadNotifications();
              ...
              <div className="flex gap-2">
                <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    ...
                    <DropdownMenuItem onClick={() => setFilterType('commande')}>
                      Commandes
                    </DropdownMenuItem>
              ...
              <div className="max-h-[400px] overflow-y-auto">
                <Tabs defaultValue="all" className="w-full">
                  <TabsList className="grid w-full grid-cols-4">
                    ...
                    <TabsTrigger value="system">Système ({counts.system})</TabsTrigger>
                  </TabsList>
                    ...
                    {filteredNotifications
                      ...
                      .map((notification) => (
                        <div
                          key={notification.id}
                          className={`p-3 rounded-lg border ${
                            notification.lu ? 'bg-gray-50' : 'bg-green-50 border-green-200'
                          }`}
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex items-start space-x-3 flex-1">
                  ...
                  <TabsContent value="system" className="space-y-2 p-2">
                    {filteredNotifications
                      .filter(n => n.type === 'systeme')
                      .map((notification) => (
                        <div
                          key={notification.id}
                          className={`p-3 rounded-lg border ${
                            notification.lu ? 'bg-gray-50' : 'bg-purple-50 border-purple-200'
                          }`}
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex items-start space-x-3 flex-1">
